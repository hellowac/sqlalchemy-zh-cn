...
.. tab:: 中文
    ...
    从版本 12 开始，Oracle 数据库可以使用 :class:`_sql.Identity` 来指定自动递增行为，如下所示::
    ...
    上面 :class:`_schema.Table` 对象的 CREATE TABLE 语句如下：
    ...
    :class:`_schema.Identity` 对象支持多种选项来控制列的“自动递增”行为，例如起始值、递增值等。除了标准选项，Oracle 数据库还支持将 :paramref:`_schema.Identity.always` 设置为 ``None``，以使用默认生成模式，这将在 DDL 中渲染为 GENERATED AS IDENTITY。Oracle 数据库还支持通过方言关键字参数指定两个自定义选项：
...
.. tab:: 中文
    ...
    * ``oracle_order``：设置为 ``True`` 时，渲染 ORDER 关键字，表示该序列是有序的。对于使用 Oracle RAC 的环境，可能需要提供确定性排序。
    ...
    .. versionchanged::  1.4   增加了 :class:`_schema.Identity` 构造函数
       用于 :class:`_schema.Column` 中，指定自动递增列的选项。
...
.. tab:: 中文
    ...
    对于 ``READ COMMITTED`` 和 ``SERIALIZABLE``，Oracle 数据库方言通过 ``ALTER SESSION`` 设置
    ...
    * ``SERIALIZABLE``
    ...
    .. note:: 由 Oracle 数据库方言实现的 :meth:`_engine.Connection.get_isolation_level` 方法
       必须强制开始事务，使用 Oracle 数据库 DBMS_TRANSACTION.LOCAL_TRANSACTION_ID 函数；
       否则，通常无法读取任何隔离级别。

       此外，若由于权限或其他原因无法访问 ``v$transaction`` 视图，
       :meth:`_engine.Connection.get_isolation_level` 方法将引发异常，
       这在 Oracle 数据库安装中是常见的情况。

       python-oracledb 和 cx_Oracle 方言会在方言首次连接数据库时调用
       :meth:`_engine.Connection.get_isolation_level` 方法，以获取“默认”的隔离级别。
       该默认级别对于在通过 :meth:`_engine.Connection.execution_options` 方法
       临时修改后重置连接上的级别非常重要。若 :meth:`_engine.Connection.get_isolation_level`
       方法因 ``v$transaction`` 无法读取或其他数据库相关失败而引发异常，
...
.. tab:: 中文
    ...
    如果你查询 Oracle 数据库关于此表的信息，表名将报告为 ``MYTABLE``，列名将报告为 ``IDENTIFIER``。与 PostgreSQL 和 MySQL 等大多数其他数据库相比，它们会将这些名称报告为 ``mytable`` 和 ``identifier``。这些名称 **没有被引号引用，因此是不区分大小写的** 。仅当它们在表定义中用引号括起来时，``MyTable`` 和 ``Identifier`` 的特殊大小写才会被保留：

    .. sourcecode:: sql

        CREATE TABLE "MyTable" ("Identifier" INTEGER PRIMARY KEY)

    在构建 SQLAlchemy :class:`.Table` 对象时， **所有小写的名称被视为不区分大小写** 。因此，以下表假定名称不区分大小写::
    ...
    对于表反射，在 Oracle 数据库的目录表中看到的全部大写的表名将被假定为不区分大小写的名称。这使得 ``Table`` 定义可以使用小写名称，并且在跨数据库视角下与 PostgreSQL 和 MySQL 等其他数据库兼容：
        ...
        # 匹配使用 CREATE TABLE mytable 创建的表
        Table("mytable", metadata, autoload_with=some_engine)

    如上所示，所有小写的名称 ``"mytable"`` 是不区分大小写的；它将与 PostgreSQL 中报告的 ``"mytable"`` 和 Oracle 中报告的 ``"MYTABLE"`` 匹配。如果没有名称规范化，上述 :class:`.Table` 定义将无法在跨数据库的方式中进行自省，因为我们处理的是一个不区分大小写的名称，而不同的数据库报告该名称的方式不同。
    ...
    对于引号全部小写的名称的特殊情况，可以使用 :class:`.quoted_name` 构造函数：
    ...
    名称规范化还适用于处理 **纯文本 SQL 字符串** 的结果集，这些 SQL 字符串没有其他 :class:`.Table` 或 :class:`.Column` 元数据与之关联。这包括使用 :meth:`.Connection.exec_driver_sql` 执行的 SQL 字符串和使用 :func:`.text` 构造函数执行的 SQL 字符串，这些 SQL 字符串不包含 :class:`.Column` 元数据。
    ...
    对于这个特殊的情况，SQLAlchemy 提供了 :paramref:`.Connection.execution_options.driver_column_names` 执行选项，它会关闭结果集的名称规范化：
    ...
    .. versionadded:: 2.1 添加了 :paramref:`.Connection.execution_options.driver_column_names`
       执行选项
...
.. tab:: 中文
    ...
    SQLAlchemy 会注意到 Oracle 数据库支持的最大标识符长度。这会影响生成的 SQL 标签名称以及约束名称的生成，尤其是在使用 :ref:`constraint_naming_conventions` 中描述的约束命名约定特性时。

    Oracle 数据库 12.2 将默认的最大标识符长度从 30 增加到 128。自 SQLAlchemy 1.4 起，Oracle 方言的默认最大标识符长度为 128 个字符。首次连接时，会获取数据库实际支持的最大长度。在所有情况下，设置 :paramref:`_sa.create_engine.max_identifier_length` 参数会绕过此更改，所提供的值将直接使用：
    ...
    在 Oracle 数据库 12.2 或更高版本上运行 SQLAlchemy 1.4 之前的版本的应用程序因此可能面临数据库迁移场景，在这种场景下，想要“DROP CONSTRAINT”一个先前使用较短长度生成的名称。当标识符长度更改时，如果索引或约束的名称没有首先调整，这个迁移将失败。强烈建议这些应用程序使用 :paramref:`_sa.create_engine.max_identifier_length` 来维护截断名称的生成控制，并在更改此值时在分阶段环境中充分审查和测试所有数据库迁移，以确保减轻此更改的影响。

    .. versionchanged:: 1.4 Oracle 数据库的默认 max_identifier_length 为 128 个字符，若 Oracle 数据库或其兼容性设置低于 12.2 版本，首次连接时会调整为 30。
...
.. tab:: 中文
    ...
    像 :meth:`_sql.Select.limit` 和 :meth:`_sql.Select.offset` 这样的方法使用了 ``FETCH FIRST N ROW / OFFSET N ROWS`` 语法，假设使用的是 Oracle 数据库 12c 或更高版本，并且假设 SELECT 语句没有嵌套在联合语句（如 UNION）中。通过使用 :meth:`_sql.Select.fetch` 方法，也可以直接使用这种语法。

    .. versionchanged:: 2.0 Oracle 数据库方言现在对所有 :meth:`_sql.Select.limit` 和 :meth:`_sql.Select.offset` 的使用，包括 ORM 和遗留的 :class:`_orm.Query`，都使用 ``FETCH FIRST N ROW / OFFSET N ROWS``。要强制使用旧版行为（使用窗口函数），请在 :func:`_sa.create_engine` 中指定 ``enable_offset_fetch=False`` 方言参数。

    通过将 ``enable_offset_fetch=False`` 传递给 :func:`_sa.create_engine`，可以禁用 ``FETCH FIRST / OFFSET``，从而强制使用基于窗口函数的“旧版”模式。对于 12c 之前的 Oracle 数据库版本，系统也会自动选择此模式。

    在使用旧版模式时，或者当包含限制/偏移的 :class:`.Select` 语句嵌套在复合语句中时，会使用基于窗口函数的模拟方法来实现 LIMIT / OFFSET，这涉及到使用 ``ROW_NUMBER`` 创建子查询，这可能会导致性能问题以及复杂语句的 SQL 构造问题。然而，这种方法在所有 Oracle 数据库版本中都受到支持。请参阅下面的注释。
...
.. tab:: 中文
    ...
    如果在 Oracle 数据库 12c 之前的版本上使用 :meth:`_sql.Select.limit` 和 :meth:`_sql.Select.offset`，或在 ORM 中使用 :meth:`_orm.Query.limit` 和 :meth:`_orm.Query.offset` 方法，以下注意事项适用：

    * SQLAlchemy 当前使用 ROWNUM 来实现 LIMIT/OFFSET；具体方法可以参考 https://blogs.oracle.com/oraclemagazine/on-rownum-and-limiting-results 。

    * 默认情况下不会使用 "FIRST_ROWS()" 优化关键字。要启用此优化指令，请在 :func:`_sa.create_engine` 中指定 ``optimize_limits=True``。

      .. versionchanged:: 1.4

          Oracle 数据库方言使用“后编译”方案呈现 limit/offset 整数值，在将语句传递给游标执行之前，直接呈现整数值。``use_binds_for_limits`` 标志不再生效。
...
.. tab:: 中文
    ...
    Oracle 数据库完全支持在 INSERT、UPDATE 和 DELETE 语句中使用 RETURNING，并且这些语句仅使用一组绑定参数（即 ``cursor.execute()`` 风格的语句；SQLAlchemy 通常不支持与 :term:`executemany` 语句一起使用 RETURNING）。多个行也可以返回。

    .. versionchanged:: 2.0 Oracle 数据库后端对 RETURNING 的支持与其他后端保持一致。
...
.. tab:: 中文
    ...
    .. warning:: 不知道 Oracle 数据库 8 的兼容性状态在 SQLAlchemy 2.0 中的表现。

    当检测到 Oracle 数据库 8 时，方言会自动配置为以下行为：

    * 将 use_ansi 标志设置为 False。这会将所有 JOIN 子句转换为 WHERE 子句，并在 LEFT OUTER JOIN 的情况下使用 Oracle 的 (+) 运算符。

    * 当使用 :class:`~sqlalchemy.types.Unicode` 时，不再生成 NVARCHAR2 和 NCLOB 数据类型的 DDL，而是生成 VARCHAR2 和 CLOB。这是因为即使这些类型可用，它们在 Oracle 8 上似乎不能正常工作。 :class:`~sqlalchemy.types.NVARCHAR` 和 :class:`~sqlalchemy.dialects.oracle.NCLOB` 类型将始终生成 NVARCHAR2 和 NCLOB。
...
.. tab:: 中文
    ...
    在使用反射与 Table 对象时，方言可以通过将 ``oracle_resolve_synonyms=True`` 作为关键字参数传递给 :class:`_schema.Table` 构造函数，选择性地查找由同义词指示的表，无论是位于本地或远程模式中，还是通过 DBLINK 访问::

        some_table = Table(
            "some_table", autoload_with=some_engine, oracle_resolve_synonyms=True
        )

    设置此标志时，给定的名称（例如上面的 ``some_table``）不仅会在 ``ALL_TABLES`` 视图中查找，还会在 ``ALL_SYNONYMS`` 视图中查找，看看这个名称是否实际上是另一个名称的同义词。如果找到了同义词并且指向 DBLINK，Oracle 数据库方言会知道如何使用 DBLINK 语法（例如 ``@dblink``）来定位该表的信息。

    ``oracle_resolve_synonyms`` 可以在接受反射参数的任何地方使用，包括 :meth:`_schema.MetaData.reflect` 和 :meth:`_reflection.Inspector.get_columns` 等方法。
...
.. tab:: 中文
    ...
    在 :class:`_schema.Table` 层级使用反射时，
    :class:`_schema.Table` 也将包括这些约束。

    请注意以下几点：

    * 使用 :meth:`_reflection.Inspector.get_check_constraints` 方法时，
      Oracle 数据库会为指定 "NOT NULL" 的列构建一个特殊的 "IS NOT NULL" 约束。
      默认情况下，此约束 **不会** 返回；要包括 "IS NOT NULL" 约束，请传递 ``include_all=True`` 标志::
    ...
    * 在大多数情况下，当反射 :class:`_schema.Table` 时，唯一约束* *不会** 作为 :class:`.UniqueConstraint` 对象提供，因为 Oracle 数据库通常用 UNIQUE 索引来镜像唯一约束（例外情况似乎是当两个或更多的唯一约束表示相同的列时）；因此，:class:`_schema.Table` 将使用 :class:`.Index` 并设置 ``unique=True`` 标志来表示这些约束。
...
.. tab:: 中文
    ...
    :meth:`_reflection.Inspector.get_table_names` 和
    :meth:`_reflection.Inspector.get_temp_table_names` 方法会返回当前引擎的表名列表。这些方法也是反射操作的一部分，例如在 :meth:`_schema.MetaData.reflect` 中执行的操作。默认情况下，这些操作会排除 ``SYSTEM`` 和 ``SYSAUX`` 表空间。如果要更改此行为，可以使用 ``exclude_tablespaces`` 参数在引擎级别更改默认排除的表空间列表::
...
.. tab:: 中文
    ...
    SQLAlchemy 的 :class:`.Float` 和 :class:`.Double` 数据类型是通用数据类型，会解析为给定后端的“最符合预期”的数据类型。
    对于 Oracle 数据库，这意味着它们解析为 ``FLOAT`` 和 ``DOUBLE`` 类型::
    ...
    Oracle 的 ``FLOAT`` / ``DOUBLE`` 数据类型是 ``NUMBER`` 的别名。Oracle 数据库存储 ``NUMBER`` 值时具有完全精度，而不是浮动点精度，这意味着 ``FLOAT`` / ``DOUBLE`` 实际上并不表现得像原生的浮点数值。Oracle 数据库则提供了特殊的数据类型 ``BINARY_FLOAT`` 和 ``BINARY_DOUBLE`` 来提供真正的 4 字节和 8 字节浮点数值。

    SQLAlchemy 通过 :class:`.BINARY_FLOAT` 和 :class:`.BINARY_DOUBLE` 直接支持这些数据类型。
    要以数据库无关的方式使用 :class:`.Float` 或 :class:`.Double` 数据类型，同时允许 Oracle 后端使用其中之一，请使用 :meth:`.TypeEngine.with_variant` 方法来设置变体::
...
.. tab:: 中文
    ...
    Oracle 数据库没有名为 ``DATETIME`` 的数据类型，只有 ``DATE`` 类型，它实际上可以存储日期和时间值。
    因此，Oracle 数据库方言提供了一个类型 :class:`_oracle.DATE`，它是 :class:`.DateTime` 的子类。此类型没有特殊行为，仅作为该类型的“标记”存在；此外，当数据库列被反射并且类型报告为 ``DATE`` 时，将使用支持时间的 :class:`_oracle.DATE` 类型。
...
.. tab:: 中文
    ...
    在与 :class:`_schema.Table` 构造结合使用时，CREATE TABLE 短语支持以下选项与 Oracle 数据库方言：
    ...
    * ``TABLESPACE``::
      ...
      ``oracle_tablespace`` 参数指定表要创建的表空间。
      当你希望在用户的默认表空间以外的表空间中创建表时，这非常有用。
